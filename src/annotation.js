...
import { Catalog, FileSpec, ObjectLoader } from 'pdfjs-dist/lib/core/obj';
import {
  Dict, isDict, isName, isRef, isStream
} from 'pdfjs-dist/lib/core/primitives';
import { ColorSpace } from 'pdfjs-dist/lib/core/colorspace';
import { getInheritableProperty } from 'pdfjs-dist/lib/core/core_utils';
import { OperatorList } from 'pdfjs-dist/lib/core/operator_list';
    ...
    switch (subtype) {
      ...
      case 'Widget':
        ...
        switch (fieldType) {
          ...
          case 'KSI':
            return new KsiWidgetAnnotation(parameters);
        }
...
class KsiWidgetAnnotation extends WidgetAnnotation {
  constructor(params) {
    super(params);

    this.setFlags(AnnotationFlag.HIDDEN);

    const value = params.dict.get('V');
    const ksiXref = params.xref.getEntry(parseInt(value.objId));
    const xrefArray = params.xref.entries.slice();
    const byteRange = value.get('ByteRange');
    xrefArray.sort((a, b) => {
      return a.offset - b.offset;
    });

    const index = xrefArray.findIndex((el) => {
      return el === ksiXref;
    });
    ...
    if (byteRange[0] !== 0) {
      warn('KSI Signature byte range is not from start of file');
      validByteRange = false;
    }

    if (sigStart !== byteRange[1] || sigEnd !== byteRange[2]) {
      warn('Element is removed or added before signature or signature has been modifed');
      validByteRange = false;
    }

    if (index > 0 && byteRange[1] < xrefArray[index - 1].offset) {
      warn('Element is between KSI signature start and ' +
        'first hash data part end');
      validByteRange = false;
    }

    if (index < xrefArray.length - 1 && byteRange[2] > xrefArray[index + 1]) {
      warn('Element is between KSI signature end and ' +
        'hash continue start position');
      validByteRange = false;
    }

    const byteRangeEnd = byteRange[2] + byteRange[3];

    if (params.pdfManager.pdfDocument.stream.end < byteRangeEnd) {
      warn('KSI signature byte range end is after EOF');
      validByteRange = false;
    }

    // TODO: There was end of line char, not sure if it should be.
    const eofBytes = params.pdfManager.pdfDocument.stream.bytes
      .slice(byteRangeEnd - 6, byteRangeEnd);

    if (!eofBytes.join().includes('37,37,69,79,70')) {
      validByteRange = false;
    }

    this.data.byteRange = byteRange;
    this.data.validByteRange = validByteRange;
    this.data.signature = stringToBytes(value.get('Contents'));
  ...
  find(uint8, needle, start, limit) {
    start = typeof start !== 'undefined' ? start : 0;
    limit = typeof limit !== 'undefined' ? limit : Number.MAX_SAFE_INTEGER;

    const search = this.stringToUint8Array(needle);
    let match = 0;
